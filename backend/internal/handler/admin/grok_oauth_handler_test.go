...
func TestGrokOAuthHandlerQueryQuotaProbesUpstream(t *testing.T) {
	...
	router := gin.New()
	router.GET("/api/v1/admin/xai/accounts/:id/quota", handler.QueryQuota)
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/admin/xai/accounts/42/quota", nil)
...
func TestGrokOAuthHandlerResetQuotaReturnsUnsupported(t *testing.T) {
	...
	router := gin.New()
	router.POST("/api/v1/admin/xai/accounts/:id/reset-quota", handler.ResetQuota)
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/v1/admin/xai/accounts/43/reset-quota", nil)
	router.ServeHTTP(rec, req)
...
func TestGrokOAuthHandlerRuntimeSanityDoesNotExposeSecrets(t *testing.T) {
	...
	router := gin.New()
	router.GET("/api/v1/admin/xai/runtime-sanity", handler.RuntimeSanity)
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/admin/xai/runtime-sanity", nil)
	router.ServeHTTP(rec, req)
