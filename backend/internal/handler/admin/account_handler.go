...
type CreateAccountRequest struct {
	...
	Type                    string         `json:"type" binding:"required,oneof=oauth setup-token apikey cookie upstream bedrock service_account"`
...
type UpdateAccountRequest struct {
	...
	Notes                   *string        `json:"notes"`
	Type                    string         `json:"type" binding:"omitempty,oneof=oauth setup-token apikey cookie upstream bedrock service_account"`
...
func (h *AccountHandler) buildOpenAIAccountSchedulerScores(ctx context.Context, accounts, filterPool []service.Account) (map[int64]*AccountSchedulerScore, map[int64][]AccountSchedulerGroupScore) {
	if len(accounts) == 0 {
		return nil, nil
	}
	if len(filterPool) == 0 {
	...
	collect := func(pool []service.Account) {
		for i := range pool {
	...
	for accountID, score := range h.scoreOpenAIAccountSchedulerPool(ctx, filterPool, loadMap) {
		copyScore := score
		baseScores[accountID] = &copyScore
	}
	groupScores := make(map[int64][]AccountSchedulerGroupScore)
	for _, groupID := range groupIDList {
		pool, ok := groupPools[groupID]
		if !ok {
			continue
		}
		name := ""
		...
		for i := range pool {
			for _, accountGroup := range pool[i].AccountGroups {
				...
				priorities[pool[i].ID] = accountGroup.Priority
		...
		for accountID, score := range h.scoreOpenAIAccountSchedulerPool(ctx, pool, loadMap) {
			if _, ok := pageIDs[accountID]; !ok {
				continue
			}
			row := AccountSchedulerGroupScore{GroupID: &gid, GroupName: name, AccountSchedulerScore: score}
			if priority, ok := priorities[accountID]; ok {
				row.GroupPriority = &priority
			}
			groupScores[accountID] = append(groupScores[accountID], row)
		}
	}
...
func (h *AccountHandler) List(c *gin.Context) {
	...
	if includeSchedulerScore {
		for i := range accounts {
			...
			filterPool := h.listAccountSchedulerScoreFilterPool(c.Request.Context(), platform, accountType, status, search, groupID, privacyMode)
			schedulerScores, schedulerGroupScores = h.buildOpenAIAccountSchedulerScores(c.Request.Context(), accounts, filterPool)
			break
		}
	}
...
func (h *AccountHandler) GetByID(c *gin.Context) {
	...
	h.enrichShadowParents(c.Request.Context(), []AccountWithConcurrency{item})
...
func (h *AccountHandler) refreshSingleAccount(ctx context.Context, account *service.Account) (*service.Account, string, error) {
	if account.IsCredentialShadow() {
		return nil, "", infraerrors.BadRequest("SPARK_SHADOW_NO_REFRESH", "cannot refresh spark shadow account; credentials are managed by the parent account")
	}
	if !account.IsOAuth() {
