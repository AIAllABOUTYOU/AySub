...
type GrokConfig struct {
	...
	DynamicStatsigEnabled      bool   `mapstructure:"dynamic_statsig_enabled"`
}
...
func setDefaults() {
	...
	viper.SetDefault("log.service_name", "aysub")
	...
	viper.SetDefault("database.dbname", "aysub")
	...
	viper.SetDefault("dashboard_cache.key_prefix", "aysub:")
...
func (c *Config) Validate() error {
	...
	if c.BatchImage.VertexEnabled {
		...
		if strings.TrimSpace(c.BatchImage.VertexManagedGCSPrefix) == "" || !strings.Contains(c.BatchImage.VertexManagedGCSPrefix, "{batch_id}") {
			return fmt.Errorf("batch_image.vertex_managed_gcs_prefix must contain {batch_id}")
		}
		if c.BatchImage.VertexInputRetentionHours <= 0 || c.BatchImage.VertexOutputRetentionHours <= 0 {
			return fmt.Errorf("batch_image.vertex retention hours must be positive")
		}
	}
