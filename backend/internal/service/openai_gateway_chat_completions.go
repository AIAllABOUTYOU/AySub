...
func shouldUseResponsesAPIForAccount(account *Account) bool {
	if account == nil {
		return true
	}
	if account.Type == AccountTypeAPIKey && account.Platform == PlatformXAI {
		return false
	}
...
//
// 当前路由策略（基于平台、账号覆盖模式/探测标记，详见 shouldUseResponsesAPIForAccount）：
//   - APIKey 账号 + 强制或探测确认不支持 Responses → 走 forwardAsRawChatCompletions
//     直转上游 /v1/chat/completions，不做协议转换
//   - xAI APIKey 账号默认走 /v1/chat/completions，避免把官方 xAI OpenAI-compatible
//     上游误判为 OpenAI Responses API
...
func (s *OpenAIGatewayService) ForwardAsChatCompletions(
	...
	if account.IsXAICookie() {
		return s.ForwardGrokChatCompletions(ctx, c, account, body, defaultMappedModel)
	}
	restrictionResult := s.detectCodexClientRestriction(c, account, body)
	logCodexCLIOnlyDetection(ctx, c, account, getAPIKeyIDFromContext(c), restrictionResult, body)
	if restrictionResult.Enabled && !restrictionResult.Matched {
		MarkOpsClientBusinessLimited(c, OpsClientBusinessLimitedReasonLocalPolicyDenied)
		c.JSON(http.StatusForbidden, gin.H{"error": gin.H{"type": "forbidden_error", "message": CodexClientRestrictionMessage(restrictionResult)}})
