...
var grokTransportFailoverBody = []byte(`{"error":{"type":"upstream_error","message":"Upstream request failed"}}`)
...
func (s *OpenAIGatewayService) getGrokAccessToken(ctx context.Context, account *Account) (string, error) {
	...
	if account == nil || !account.IsGrok() {
		return "", errors.New("not a grok account")
	}
	switch account.Type {
	...
	case AccountTypeOAuth:
		...
		if expiresAt := account.GetCredentialAsTime("expires_at"); expiresAt != nil && !time.Now().Before(*expiresAt) {
			return "", errors.New("grok access_token expired; token provider refresh is required")
		}
		return token, nil
	default:
...
func (s *OpenAIGatewayService) handleOpenAIUpstreamTransportError(_ context.Context, c *gin.Context, account *Account, err error, passthrough bool) error {
	...
	return &UpstreamFailoverError{StatusCode: http.StatusBadGateway, ResponseBody: grokTransportFailoverBody}
}
