...
func (s *GrokQuotaService) probeUsage(ctx context.Context, accountID int64) (*GrokQuotaProbeResult, error) {
	...
	resp, err := s.httpUpstream.Do(req, proxyURL, account.ID, grokMaxInt(account.Concurrency, 1))
	if err != nil {
	...
	if resp.StatusCode >= 400 {
		...
		return nil, infraerrors.Newf(grokMapUpstreamStatus(resp.StatusCode), "GROK_QUOTA_PROBE_UPSTREAM_ERROR", "upstream returned %d for probe model %q: %s", resp.StatusCode, probeModel, bodyText)
	}
	return result, nil
}
...
func (s *GrokQuotaService) fetchBilling(
	...
	resp, err := s.httpUpstream.Do(req, proxyURL, account.ID, grokMaxInt(account.Concurrency, 2))
	if err != nil {
	...
	if resp.StatusCode >= 400 {
		...
		return nil, resp.StatusCode, infraerrors.Newf(grokMapUpstreamStatus(resp.StatusCode), "GROK_QUOTA_PROBE_UPSTREAM_ERROR", "billing returned %d: %s", resp.StatusCode, bodyText)
	}
...
func mergeGrokBillingProbeErrors(weeklyStatus, monthlyStatus int, weeklyErr, monthlyErr error) error {
	...
	if weeklyKey == monthlyKey {
		...
		case weeklyStatus != 0 && weeklyStatus != http.StatusOK:
			return infraerrors.New(grokMapUpstreamStatus(weeklyStatus), "GROK_QUOTA_PROBE_UPSTREAM_ERROR", "xAI billing endpoints returned the same upstream error")
		default:
...
// grokMapUpstreamStatus collapses upstream HTTP statuses into a stable set we
...
func grokMapUpstreamStatus(status int) int {
	switch {
