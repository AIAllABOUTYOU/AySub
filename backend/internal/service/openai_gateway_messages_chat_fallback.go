...
// forwardAnthropicViaRawChatCompletions serves Anthropic Messages compatibility
...
func (s *OpenAIGatewayService) forwardAnthropicViaRawChatCompletions(
	...
	if err != nil {
		...
		return nil, fmt.Errorf("convert anthropic responses request to chat completions: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("marshal anthropic chat completions fallback request: %w", err)
	}
...
func (s *OpenAIGatewayService) streamChatCompletionsAsAnthropic(
	...
	writeAnthropicEvents := func(events []apicompat.ResponsesStreamEvent) {
	...
	writeFinalAnthropicEvents := func(events []apicompat.AnthropicStreamEvent) {
		if clientDisconnected || len(events) == 0 {
			return
		}
		for _, evt := range events {
			...
			if _, err := fmt.Fprint(c.Writer, sse); err != nil {
				clientDisconnected = true
				logger.L().Debug("openai messages chat fallback: client disconnected during final flush",
					zap.Error(err),
					zap.String("request_id", requestID),
				)
				return
	...
	for scanner.Scan() {
		...
		writeAnthropicEvents(apicompat.ChatCompletionsChunkToResponsesEvents(&chunk, responsesState))
	}
	...
	writeAnthropicEvents(apicompat.FinalizeChatCompletionsResponsesStream(responsesState))
	writeFinalAnthropicEvents(apicompat.FinalizeResponsesAnthropicStream(anthropicState))
	if !sawDone {
