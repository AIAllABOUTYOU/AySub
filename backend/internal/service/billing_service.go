...
func (s *BillingService) CalculateCostUnified(input CostInput) (*CostBreakdown, error) {
	if input.Resolver == nil {
		...
		if input.LongContextBillingEnabled != nil {
			applyLongContext = *input.LongContextBillingEnabled
		}
		return s.calculateCostInternalWithPolicy(input.Model, input.Tokens, input.RateMultiplier, input.ServiceTier, nil, applyLongContext)
	}
...
func (s *BillingService) computeTokenBreakdown(
	...
	bd := &CostBreakdown{LongContextBillingApplied: longContextApplied}
...
func (s *BillingService) calculateCostInternalWithPolicy(model string, tokens UsageTokens, rateMultiplier float64, serviceTier string, channelPricing *ChannelModelPricing, applyLongContext bool) (*CostBreakdown, error) {
...
func (s *BillingService) applyModelSpecificPricingPolicy(model string, pricing *ModelPricing) *ModelPricing {
	...
	if !isGPT56 && !isOpenAIGPT54Model(normalized) {
		return pricing
	}
	...
	if !needsLongContext && !needsCacheCreation {
		return pricing
	}
	cloned := *pricing
