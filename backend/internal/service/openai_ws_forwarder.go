...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	ApplyCustomHeaderValues(headers, account)
...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	if isCodexSparkModel(mappedModel) {
		stripCodexSparkImageGenerationTools(payload)
	}
	...
	if headerErr != nil {
		return nil, wrapOpenAIWSFallback("build_headers", headerErr)
	}
	logOpenAIWSModeDebug(
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(raw []byte) (openAIWSClientPayload, error) {
		...
		if isCodexCLI && codexImageGenerationExplicitToolPolicy == codexImageGenerationExplicitToolPolicyStrip {
			...
			if changed {
				normalized = stripped
				logOpenAIWSModeInfo("ingress_ws_codex_image_declarations_stripped account_id=%d", account.ID)
			}
		}
		codexBridgeEnabled := isCodexCLI && imageGenerationAllowed && codexImageGenerationExplicitToolPolicy != codexImageGenerationExplicitToolPolicyStrip && !isOpenAIResponsesLiteWebSocketPayload(normalized) && s.isCodexImageGenerationBridgeEnabled(ctx, account, apiKey)
