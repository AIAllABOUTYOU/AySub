...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	...
	if resetTimestamp == "" {
		...
		// Anthropic 平台：没有限流重置时间的 429 不适合按长窗口封禁，
		// 但完全不标记会让调度器反复撞同一批持续 429 的账号。
...
// HandleUpstreamModelNotFound cools down deterministic model capability
// failures per (account, model), allowing the current request to fail over.
