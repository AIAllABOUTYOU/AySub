...
func TestOpenAIGatewayService_OAuthLegacy_CompositeCodexUAUsesCodexOriginator(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, upstream.lastReq)
	wantOriginator, wantUA, ok := openai.PairCodexClientIdentity(upstream.lastReq.Header.Get("user-agent"))
	require.True(t, ok)
	require.Equal(t, wantOriginator, upstream.lastReq.Header.Get("originator"))
...
func TestOpenAIGatewayService_OAuthPassthrough_NonCodexUAFallbackToCodexUA(t *testing.T) {
	...
	require.Equal(t, false, gjson.GetBytes(upstream.lastBody, "store").Bool())
	require.Equal(t, true, gjson.GetBytes(upstream.lastBody, "stream").Bool())
	require.Equal(t, "codex_cli_rs/0.144.1", upstream.lastReq.Header.Get("User-Agent"))
}
...
func TestOpenAIGatewayService_CodexCLIOnly_AllowOfficialClientFamilies(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.Request.Header.Set("x-codex-window-id", "test-window")
			if tt.originator != "" {
