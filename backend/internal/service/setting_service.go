...
// GetDefaultPlatformQuotas 读取系统全局 platform quota JSON key。
// 永远返回包含全部 AllowedQuotaPlatforms key 的 map（值可能为零值/nil 字段，表示"上层未配置 = 不限制"）。
//
...
func (s *SettingService) GetDefaultPlatformQuotas(ctx context.Context) (map[string]*DefaultPlatformQuotaSetting, error) {
	...
	return out, nil // 补齐全部 platform key，保持下游契约稳定
}
