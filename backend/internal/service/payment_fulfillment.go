...
func (s *PaymentService) acquirePaymentFulfillmentLease(ctx context.Context, o *dbent.PaymentOrder) (*paymentFulfillmentLease, error) {
	...
	// Keep the process location for SQLite-backed development/tests. PostgreSQL
	// compares timestamptz values by instant, while SQLite compares encoded time
	// values and can misorder mixed local/UTC offsets.
