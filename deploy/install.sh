#!/bin/bash
#
# AySub Installation Script
# AySub 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/AIAllABOUTYOU/AySub/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="AIAllABOUTYOU/AySub"
INSTALL_DIR="/opt/aysub"
SERVICE_NAME="aysub"
SERVICE_USER="aysub"
CONFIG_DIR="/etc/aysub"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="AySub 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 AySub 服务："
    ...
    ["not_installed"]="AySub 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 AySub。"
    ...
    ["uninstall_complete"]="AySub 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 AySub"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 AySub"
    ...
    ["server_config_desc"]="配置 AySub 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="AySub Installation Script"
    ...
    # Completion
    ["install_complete"]="AySub installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start AySub service:"
    ...
    ["not_installed"]="AySub is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove AySub from your system."
    ...
    ["uninstall_complete"]="AySub has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install AySub"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove AySub"
    ...
    ["server_config_desc"]="Configure AySub server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/aysub" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/aysub" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="aysub_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/aysub" "$INSTALL_DIR/aysub"
    chmod +x "$INSTALL_DIR/aysub"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/aysub"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/aysub.service << EOF
[Unit]
Description=AySub - AI API Gateway Platform
Documentation=https://github.com/AIAllABOUTYOU/AySub
...
User=aysub
Group=aysub
WorkingDirectory=/opt/aysub
ExecStart=/opt/aysub/aysub
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=aysub
...
enable_autostart() {
    ...
    if systemctl enable aysub 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status aysub"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u aysub -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart aysub"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop aysub"
    echo ""
    echo "=============================================="
}
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/aysub" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet aysub; then
        print_info "$(msg 'stopping_service')"
        systemctl stop aysub
    fi

    # Backup current binary
    cp "$INSTALL_DIR/aysub" "$INSTALL_DIR/aysub.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/aysub.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/aysub"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start aysub

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: AySub must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/aysub" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="aysub.backup.${current_version}"
        else
            backup_name="aysub.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/aysub" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop aysub 2>/dev/null || true
    systemctl disable aysub 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/aysub.service
    systemctl daemon-reload
