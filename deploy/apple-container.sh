...
ENV_FILE="${AYSUB_ENV_FILE:-${SCRIPT_DIR}/.env}"

STACK_LABEL_KEY="org.aysub.stack"
STACK_LABEL_VALUE="apple-container"
NETWORK_NAME="aysub-apple"
APP_CONTAINER="aysub-apple"
POSTGRES_CONTAINER="aysub-apple-postgres"
REDIS_CONTAINER="aysub-apple-redis"
APP_VOLUME="aysub-apple-data"
POSTGRES_VOLUME="aysub-apple-postgres-data"
REDIS_VOLUME="aysub-apple-redis-data"
PLATFORM="linux/arm64"

TEMP_DIR=""
LOCK_DIR="${TMPDIR:-/tmp}/aysub-apple-container.lock"
LOCK_ACQUIRED=false
...
Commands:
  ...
  up [--recreate]       Create and start the complete AySub stack
...
Environment:
  AYSUB_ENV_FILE        Path to the deployment env file (default: deploy/.env)
EOF
}
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another AySub Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
cmd_init() {
    ...
    info "Review the file, then run: AYSUB_ENV_FILE='${ENV_FILE}' ${SCRIPT_DIR}/apple-container.sh up"
}
...
prepare_environment() {
    ...
    APP_IMAGE="$(read_env_value APPLE_CONTAINER_AYSUB_IMAGE ghcr.io/aiallaboutyou/aysub:latest)"
    ...
    POSTGRES_USER="$(read_env_value POSTGRES_USER aysub)"
    POSTGRES_PASSWORD="$(read_env_value POSTGRES_PASSWORD)"
    POSTGRES_DB="$(read_env_value POSTGRES_DB aysub)"
    ...
    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/aysub-apple.XXXXXX")"
...
create_app_container() {
    ...
    container create \
        ...
        --entrypoint /bin/sh \
        "${APP_IMAGE}" \
        -c 'set -e; mkdir -p "$DATA_DIR"; chown -R aysub:aysub "$DATA_DIR"; exec su-exec aysub /app/aysub' \
        >/dev/null
}
...
start_app() {
    ...
    if ! wait_for_probe "AySub" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "AySub did not become ready."
    fi
    if ! wait_for_probe "AySub host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "AySub is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "AySub stack stopped; persistent volumes were preserved."
}
...
cmd_logs() {
    ...
    case "${service}" in
        app|aysub) container_name="${APP_CONTAINER}" ;;
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the AySub stack and all persistent data? [y/N] '
    else
        printf 'Delete the AySub containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "AySub stack and persistent data deleted."
    else
        info "AySub stack deleted; persistent volumes were preserved."
    fi
}
