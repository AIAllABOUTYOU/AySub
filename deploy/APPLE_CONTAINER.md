...
AySub can run as a native three-service stack with Apple's `container` CLI. This workflow runs the published AySub, PostgreSQL, and Redis OCI images without Docker Desktop or a Docker-compatible daemon.
...
# Creates volumes/network/containers, waits for dependencies, and starts AySub.
...
# Restart PostgreSQL, Redis, and AySub in dependency order.
...
The script uses `deploy/.env`, the same source file used by Docker Compose. Export `AYSUB_ENV_FILE` to use another file for every command in the current shell:

```bash
export AYSUB_ENV_FILE=/absolute/path/to/aysub.env
./apple-container.sh init
./apple-container.sh up
```
...
```dotenv
APPLE_CONTAINER_AYSUB_IMAGE=ghcr.io/aiallaboutyou/aysub:latest
...
|---|---|
| Application and gateway variables | Passed to AySub from `.env` |
...
| `REDIS_PASSWORD` | Applied to Redis and AySub |
...
The script creates only resources carrying the `org.aysub.stack=apple-container` label:

| Type | Names |
|---|---|
| Containers | `aysub-apple`, `aysub-apple-postgres`, `aysub-apple-redis` |
| Network | `aysub-apple` |
| Volumes | `aysub-apple-data`, `aysub-apple-postgres-data`, `aysub-apple-redis-data` |

The PostgreSQL volume is mounted at `/var/lib/postgresql`, retaining PostgreSQL 18's default child data directory. AySub and Redis also store data in child directories below their Apple volume mount points. This is required because Apple named volumes do not have Docker's copy-up and mount-point ownership behavior.

## Networking

Apple `container` 1.1 does not provide Compose-style network-scoped service aliases. After PostgreSQL and Redis start, the script reads their current private-network IPv4 addresses from `container inspect`, injects those addresses into a newly created application container, and then starts AySub. The script does not modify `~/.config/container/config.toml` or the macOS host resolver.

All three services attach only to the private `aysub-apple` network. Only the application publishes a host port; database and Redis ports remain unpublished.

The application container is intentionally recreated by every `up` and `restart` operation because dependency VM addresses can change after they stop. Application data remains in `aysub-apple-data`.
...
container exec aysub-apple sh -c \
...
container exec aysub-apple sh -c 'tar -C "$DATA_DIR" -czf - .' \
  > backups/aysub-data.tar.gz
...
container delete aysub-apple
AYSUB_IMAGE=ghcr.io/aiallaboutyou/aysub:latest # Match APPLE_CONTAINER_AYSUB_IMAGE in .env.
container run --rm --name aysub-apple-data-restore \
  --entrypoint /bin/sh \
  --volume aysub-apple-data:/restore \
  --volume "$PWD/backups:/backup:ro" \
  "$AYSUB_IMAGE" \
  -c 'rm -rf /restore/data && mkdir -p /restore/data && tar -xzf /backup/aysub-data.tar.gz -C /restore/data'

# Restore the logical database while the application is absent.
container start aysub-apple-postgres
until container exec aysub-apple-postgres sh -c 'pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB"'; do sleep 1; done
container copy backups/aysub.sql aysub-apple-postgres:/tmp/aysub.sql
container exec aysub-apple-postgres sh -c '
  export PGPASSWORD="$POSTGRES_PASSWORD"
  dropdb -h 127.0.0.1 -U "$POSTGRES_USER" --if-exists --force "$POSTGRES_DB"
  createdb -h 127.0.0.1 -U "$POSTGRES_USER" "$POSTGRES_DB"
  psql -h 127.0.0.1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -f /tmp/aysub.sql
  rm /tmp/aysub.sql
'
...
- The script does not mount the optional host `datamanagementd` Unix socket, so AySub's data-management integration is unavailable in this stack.
- Named volume backup and restore must be tested before using this workflow for important data.
- The script targets native `linux/arm64` images. The normal AySub release publishes an arm64 variant.
