...
describe("admin settings auth source defaults helpers", () => {
  ...
  it("reads nested platform_quotas from settings into auth source state", () => {
    ...
    // 未出现的平台（xai/gemini/antigravity）归一化为 null
    expect(state.email.platform_quotas.xai).toEqual({ daily: null, weekly: null, monthly: null });
  ...
  it("appends sanitized nested platform_quotas with non-null values in update payload", () => {
    ...
    // 缺失平台归一化为全 null
    expect(emailQuotas.xai).toEqual({ daily: null, weekly: null, monthly: null });
...
describe("normalizePlatformQuotasMap", () => {
  it("填充缺失的平台为全 null 三档", () => {
    ...
    expect(result.xai).toEqual({ daily: null, weekly: null, monthly: null });
