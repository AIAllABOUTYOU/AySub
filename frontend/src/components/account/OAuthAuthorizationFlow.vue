...
const isLocalCallbackOAuth = computed(() => props.platform === 'openai' || props.platform === 'xai')
...
const getOAuthKey = (key: string) => {
  ...
  if (props.platform === 'xai') return `admin.accounts.oauth.grok.${key}`
...
const oauthImportantNotice = computed(() => {
  ...
  if (props.platform === 'xai') return t('admin.accounts.oauth.grok.importantNotice')
  return ''
})
...
// Auto-extract code and state from callback URLs.
// e.g., http://localhost:8085/callback?code=xxx...&state=...
watch(authCodeInput, (newVal) => {
  if (!['openai', 'gemini', 'antigravity', 'xai'].includes(props.platform)) return
