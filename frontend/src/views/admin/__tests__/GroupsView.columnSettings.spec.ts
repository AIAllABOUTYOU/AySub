...
const messages: Record<string, string> = {
  ...
  'admin.groups.limitWeek': 'w',
  'admin.groups.limitMonth': 'mo',
...
const DataTableStub = {
  ...
  template: `
    <div>
      ...
      <div v-if="data.length" data-test="billing-cell">
        <slot name="cell-billing_type" :row="data[0]" />
      </div>
    </div>
  `,
}
...
const columnKeys = (wrapper: Awaited<ReturnType<typeof mountView>>) =>
...
describe('admin GroupsView column settings', () => {
  beforeEach(() => {
    ...
    getUsageSummary.mockResolvedValue([
      { group_id: 1, today_cost: 82, total_cost: 150 },
    ])
  ...
  it('applies saved hidden columns and ignores unknown or fixed keys', async () => {
  ...
  it('skips summary fetches until a consuming column is shown', async () => {
    localStorage.setItem(
      'group-hidden-columns',
  ...
  it('shows used daily quota and total usage in the billing column', async () => {
    const wrapper = await mountView()
    const billingCell = wrapper.get('[data-test="billing-cell"]')

    expect(billingCell.text()).toContain('$82.00')
    expect(billingCell.text()).toContain('$100.0/d')
    expect(billingCell.text()).toContain('Total $150.0')
    expect(billingCell.find('.text-amber-600').exists()).toBe(true)
  })
})
