...
describe('channel monitor Grok provider', () => {
  ...
  it('offers Grok in the responsive provider grid and prefills its official defaults', async () => {
    ...
    expect(PROVIDERS).toContain(PROVIDER_XAI)
    ...
    const grokButton = wrapper.get('[data-testid="monitor-provider-xai"]')
    expect(grokButton.find('svg').exists()).toBe(true)
    expect(grokButton.text()).toContain('monitorCommon.providers.xai')
    ...
    expect((endpoint.element as HTMLInputElement).value).toBe(DEFAULT_XAI_ENDPOINT)
    expect((model.element as HTMLInputElement).value).toBe(DEFAULT_XAI_MODEL)
